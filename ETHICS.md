# ETHICAL USE TERMS
## Binding Conditions on the Use of This Software and Its Derivatives
### Vladyslav Savytskyy · Buenos Aires · 2026

---

## SCOPE

These terms apply to all code, algorithms, mathematical constructions,
kernel implementations, and derivative works contained in this repository
and all associated repositories by the same author, including but not
limited to:

- `kernel/goldberg_kernel.js` — the computational geometry kernel
- `kernel/graph_axioms.js` — the 7-primitive axiomatic layer
- All visualization, simulation, and exploration tools in `shell/`
- All research documents in `research/`
- The Goldberg-Coxeter refinement implementation
- The graph automaton and its primitive operations (P1–P7)
- The Möbius transform engine
- Any port, translation, or re-implementation of the above in any
  programming language, framework, or runtime environment

---

## PERMITTED USE

This software may be used for:

- Scientific research and academic study
- Education and teaching
- Personal exploration and learning
- Art, visualization, and creative expression
- Open-source software development
- Commercial applications that benefit human welfare
- Games, simulations, and interactive experiences
- Medical research and healthcare applications
- Environmental monitoring and conservation
- Space exploration and astronomical research

---

## PROHIBITED USE

This software, its algorithms, kernel architecture, mathematical
constructions, and any derivative works SHALL NOT be used for:

### 1. WEAPONS AND HARM
- Development of weapons of any kind (kinetic, chemical, biological,
  nuclear, radiological, cyber, or autonomous)
- Military targeting, surveillance, or kill-chain systems
- Autonomous systems designed to cause physical harm to humans or animals
- Enhancement of existing weapons systems or military infrastructure

### 2. SURVEILLANCE AND OPPRESSION
- Mass surveillance of populations
- Social credit scoring systems
- Tools designed to suppress political dissent or free expression
- Systems designed to discriminate based on race, ethnicity, religion,
  gender, sexuality, disability, or national origin
- Predictive policing that targets communities rather than evidence

### 3. DECEPTION
- Generation of disinformation or deepfakes
- Systems designed to manipulate democratic processes
- Fraudulent financial instruments or market manipulation
- Impersonation of individuals or institutions

### 4. EXPLOITATION
- Exploitation of children in any form
- Human trafficking or forced labor systems
- Environmental destruction beyond regulatory compliance
- Extraction of personal data without informed consent

---

## THE KERNEL CLAUSE

The computational kernel (`kernel/goldberg_kernel.js`) implements
mathematical operations that are, by their nature, universal and
cannot be restricted. Euler's formula, the Gauss-Bonnet theorem,
and trivalent graph theory belong to humanity.

However, the SPECIFIC IMPLEMENTATION — the architecture, the
refinement pipeline, the state management, the axiomatic decomposition
into 7 primitives — is a creative work subject to these terms.

You may not use this implementation to build systems that harm people.
You may reimplement the mathematics from scratch (it's in every textbook).
But if you use THIS CODE, these terms apply.

---

## ENFORCEMENT

These terms are not merely aspirational. They are conditions of the
GPL v3 license grant. Use of this software in violation of these terms
constitutes a breach of the license and terminates all rights granted
under it.

If you discover a violation of these terms, please contact the author.

---

## THE HONEST PART

We know these terms cannot prevent all misuse. Someone with resources
and intent will do what they want regardless of what a markdown file says.

But we are putting this here, in public, timestamped and version-controlled,
so that when the question is asked — "did they know? did they try?" — the
answer is: yes. We knew. We tried.

The math is open because math should be open.
The terms are here because tools should not be weapons.

634 lines of JavaScript should not be used to hurt anyone.
That shouldn't need to be said. But here we are.

---

## APPLICABLE REPOSITORIES

These terms apply to all repositories by Vladyslav Savytskyy:

- `vsavytsk1/Mnetv1` — MachineNet, Sacred Math Tree, Genesis
- `vsavytsk1/Mnet` — Fractal Geometry Builder
- `vsavytsk1/SpookyPrimes` — Dodecahedron of Open Questions
- All forks and derivatives of the above

---

*"The geometry does not care about your feelings."*
*"But the person who wrote it does."*

*Vladyslav Savytskyy · Buenos Aires · May 2026*
