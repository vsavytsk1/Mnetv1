...
print("VALE v1.1 -- polar window layout -- KERNELIC_MAGIC Pattern 2")
...
// BREATHE LOOP -- lightweight: only moves windows + lines, never redraws ring
function startBreathe() {
  ...
  (function loop() {
    ...
    var R = _breathe.R_base + pulse;
    var W = window.innerWidth, H = window.innerHeight;
    // only move windows -- no ring redraw, no canvas resize
    LAY.wins.forEach(function(id, i) {
      var el = document.getElementById('win-' + id);
      if (!el) return;
      var a = LAY.angles[i] - Math.PI / 2;
...
window.addEventListener('resize', function() { _ringDrawn = false; layoutApply(); });
...
// resize handled in startBreathe block above
