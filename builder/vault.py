...
# -*- coding: utf-8 -*-
# ============================================================================
#  vault.py -- the MAINFRAME discipline, mirrored from HELENA's redundancy.py
# ----------------------------------------------------------------------------
#  The rule Vlad set: no matter how small or big, WE TRIPLICATE.
#
#  Source spec: SpiderEngineering/Eleni/builder/Helena/redundancy.py (the
#  COBOL vault). Ideas cross the repo boundary; payload does not. Helena
#  vaults typed arrays (.f32/.i32/.f64) and votes per ELEMENT; our artifacts
#  are JSON dumps, which have no element type, so THE ATOM HERE IS THE BYTE
#  -- declared, not assumed. Otherwise the structure is hers:
#
...
#  Gos/runs/ dumps that git never holds. The MANIFEST is tracked (Curse 31:
#  the steps, not the payload); the copies are local.
#
#      py -3 builder/vault.py selftest              # flip a bit, heal it, prove it
#      py -3 builder/vault.py save   FILE [FILE..]  # write the 3-format vault
#      py -3 builder/vault.py verify FILE           # every copy vs manifest
#      py -3 builder/vault.py repair FILE           # TMR-vote and heal
# ============================================================================
import base64
import hashlib
import json
