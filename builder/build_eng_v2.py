...
FEATURED_FAMILIES = [
    ...
    ("genesis_v8_5_1",  "GENESIS v8.5.1", "the fractal space explorer -- HEAR THE NET, on the BIG shells, panels docked at any width. THE GOLDEN CATALOGUE: seed the next 7 buckyballs C20->C60->C140->C380->C980->C2580->C6740 (Goldberg-Coxeter, Thea Lane B -- each kernel-CERTIFIED V=20T E=30T P=12 chi=2, all-trivalent, C6740 live). FLIGHT EXPLORER centers any point on click (now docks cleanly above the command bar on half-screen too). hit LIGHT: the net sinks to BACKGROUND (still fully computed -- the price is paid -- just invisible) so you see ONLY the light FLOWING along the edges, brightness = the modal current |va-vb|, tempo = sqrt(lambda_2) (C20 = 3-sqrt5 = 2/phi^2, kernel-verified: the honest sound of the net). the light is aesthetic over the point-and-line substrate. perfect-math block: the graph wave u''=-c^2 L u conserves energy to 1e-15 (leapfrog). M1-M6 kernel live."),
