...
FEATURED_FAMILIES = [
    ...
    ("genesis_v8_4_1",  "GENESIS v8.4.1", "the fractal space explorer -- now you can HEAR THE NET. THE GOLDEN CATALOGUE: seed the next 7 buckyballs C20->C60->C140->C380->C980->C2580->C6740 (Goldberg-Coxeter, Thea Lane B -- each kernel-CERTIFIED V=20T E=30T P=12 chi=2, all-trivalent, C6740 live). the FLIGHT EXPLORER locks every point on seed + centers any pentagon/vertex on click. and the LIGHT button: the net vibrates in its true graph-Laplacian fundamental mode -- pixel-thin light FLOWS along every edge, brightness = the modal current |va-vb|, tempo = sqrt(lambda_2) (C20 = 3-sqrt5 = 2/phi^2, kernel-verified, the honest sound of the net). the light matrix, made audible. M1-M6 kernel live."),
